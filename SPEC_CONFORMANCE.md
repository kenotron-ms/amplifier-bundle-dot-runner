...
| ULM-17 | Gemini structured output has NO structural "no extra keys" guarantee — our ULM-14 sanitizer strips `additionalProperties` before sending, so `additionalProperties:false` is prompt-enforced only (OpenAI=API-strict, Anthropic=tool-schema). Did NOT leak in the eval, but the guarantee is absent. | live eval (S6, latent) | `adapters/gemini.py` `_sanitize_gemini_schema()` | OPEN | DIVERGE (documented sanitizer trade-off; follow-up = adversarial extra-keys test) |
